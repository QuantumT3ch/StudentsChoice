...
bool Map::is_hazard(glm::vec3 position) {
    ...
    //checks if tile is spikes
    //if (tile == 75 || tile == 76) return true;
    //checks for lava
    //if (tile == 1 || tile == 16|| tile == 31) return true;
    return false;
}
...
bool Map::is_solid(glm::vec3 position, float* penetration_x, float* penetration_y)
{
    ...
    *penetration_y = 0;
    ...
    //open doors
    if (tile == 36 || tile == 37 || tile == 38) return false;
    //floor
    if (tile == 48|| tile == 49 || tile == 50 || tile == 51 || tile == 52 || tile == 53 || tile == 42 || tile == 30 ) return false;
    //stairs
    if (tile == 36 || tile == 37 || tile == 38) return false;
