...
#include <vector>
enum EntityType { PLATFORM, PLAYER, ENEMY, WEAPON, ITEM  };
enum AIType     { BAT, GUARD, FLYER       };
...
public:
    ...
    void const check_collision_y(float delta_time, std::vector<Entity*> collidable_entities, int collidable_entity_count);
    void const check_collision_x(float delta_time, std::vector<Entity*> collidable_entities, int collidable_entity_count);
    ...
    void attack_up();
    void attack_down();
    void follow_player(glm::vec3 position, Entity* player);
    ...
    bool const was_damaged() const { return m_was_damaged; }
    float const get_i_frames() const { return m_invincibility_time; }
    bool const is_dead() const { return m_is_dead; }
    bool const is_attacking() const { return m_attacking; }
    bool const is_attacking_right() const { return m_attacking_right; }
    bool const is_attacking_left() const { return m_attacking_left; }
    bool const is_attacking_up() const { return m_attacking_up; }
    bool const is_attacking_down() const { return m_attacking_down; }
    float const get_death_height() const{ return death_height; }
    Entity* const get_weapon() const { return m_weapon; }
    ...
    void const  set_weapon(Entity* new_weapon)  { m_weapon = new_weapon; }
