...
void ShaderProgram::load(const char *vertex_shader_file, const char *fragment_shader_file)
{
    ...
    m_player_damaged_uniform    = glGetUniformLocation(m_program_id, "playerDamaged");
    m_lights_on_uniform         = glGetUniformLocation(m_program_id, "lightsOn");
...
void ShaderProgram::player_took_damage(int took_damage)
{
    glUseProgram(m_program_id);
    glUniform1i(m_player_damaged_uniform, took_damage);
}

void ShaderProgram:: turn_lights_on(int lights) {
    glUseProgram(m_program_id);
    glUniform1i(m_lights_on_uniform, lights);
}
